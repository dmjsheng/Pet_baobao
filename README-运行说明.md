...
这是爆爆的 Windows 桌面宠物工程。它是透明悬浮窗口：可拖动、始终置顶，点击爆爆可抚摸；悬停后可喂食、踩奶、陪伴或睡觉；右键可重置位置或退出。所有互动状态仅保存在当前 Windows 用户的本地应用数据目录，不联网、不上传照片、不自动开机启动。

## 安静陪伴行为

- 全程静音，不播放猫叫、提示音或系统通知。
- 多组透明逐帧动画：待机眨眼、抚摸、吃零食、踩奶、端坐陪伴、蜷睡等。
- 点击「陪伴」会切换端正坐姿并缓慢眨眼；点击「睡觉」会蜷成一团闭眼，再点一次醒来。
- 点击「踩奶」会端坐桌面，交替踩动前爪并自然眨眼（约 1.84 秒）。
- 睡着时抚摸、喂食、陪伴或踩奶，会先唤醒再响应互动；自主睡眠与重启后恢复睡眠也会显示蜷睡姿势。
...
- `assets\baobao\frames`：逐帧动画资源（含踩奶、端坐、蜷睡等）；随便携版一同复制，移植时不需要额外下载。
...
当前发布版为 **V1.5**。完整说明与下载见根目录 [README.md](./README.md) 与 [GitHub Releases](https://github.com/dmjsheng/Pet_baobao/releases)。
