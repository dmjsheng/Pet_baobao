...
3. Crop the four 627×627 cells, then uniformly scale each full cell canvas to 512×512; this preserves the cat's aspect ratio because both axes use the same scale.
...
- Produces: alignment checks that anchor the top/face region for these two local-paw actions. Grooming holds center ≤ 3 px, width ≤ 2%, height ≤ 3%; stretching holds center ≤ 3 px while permitting the intentional extended-paw outline (width ≤ 5%, height ≤ 13%).
